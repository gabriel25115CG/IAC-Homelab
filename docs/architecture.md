...
3. **App of apps** : l'`ApplicationSet` scanne en continu le dossier `kubernetes/apps/` du repo `IAC-Homelab` (branche `master`). Chaque sous-dossier devient automatiquement une `Application` ArgoCD synchronisée (`prune: true`, `selfHeal: true`).
